...
* Calculate the the ordinary residuals, plot the ordinary residuals against the fitted values, plot a histogram of the ordinary residuals, describe what you see. -- 1 point(s)
* Based on what you see in the residuals, what can you say about the model's assumptions of normality and heteroskedasticity? -- 0.5 point(s)
...
The residuals measure how accurately a fitted model predicts the observed data, and they are also used to calculate the coefficient of determination (i.e., R-squared). A model's R-squared value is the proportion of the variance in the response variable that is predictable from the predictor(s). You will often see researchers say something like our model explained 87% of the variance in our response variable'. In most cases they are getting this number from a model's R-squared. This can be informative, however, a challenge with R-squared values is that they don't take into account the number of parameters in the model. If we keep adding parameters to a model to soak up some of the residual variation, the R-square will necessarily decrease, which can trick us into thinking we have a better model even if the parameters are not biologically meaningful.


1. 5 points <ol>
* Calculate the R-squared value for the full model you just fit to the bird species abundance data. -- 1 point(s)
...
* Generate 12 randomly distributed numbers from a distribution of your choice. Add these as an additional column to your dataset, and refit the model you just fit but now include these random numbers. Describe the effect on the R-squared. -- 2 point(s)
</ol>
...
1. 3 points <ol>
* Compare the adjusted R-squared value you obtained in exercise 2, with that from the model with the random noise. -- 1 point(s)
* How do R-squared and adjusted R-squared compare in their ability to assess model performance. -- 2 point(s)
</ol>
...
1. 4 points <ol>
* Describe why the following function would generate heteroskedastic data:  -- 1 point(s)
```{r}
hetero <- function(x) {
  B_0 <- 0
  B_1 <- 2
...
```
* Generate 400 uniformly sampled x values between 1 and 60, and using this function generate 400 corresponding y values.  -- 0.5 point(s)
* Plot the relationship between x and y and describe what you see. -- 0.5 point(s)
* Fit a simple linear model to these data. -- 0.5 point(s)
* Plot the residuals against the fitted values. -- 0.5 point(s)
* What do these residuals tell you about the model's assumptions? -- 1 point(s)
</ol>
...
1. 3 points <ol>
* Attempt to fix the heteroscedasticity in these data by applying a transformation to the y values. -- 0.5 point(s)
...
* What does this tell you about the capacity to handle heteroskedasticity via transformations? -- 1 point(s)
</ol>
